/**
 * Emit an OpenAPI 3.1 document describing snapshot endpoints and any action
 * endpoints declared on snapshots' actions — framework-neutral.
 *
 ...
 * {@link snapshotsToOpenApi}. Adapters keep only the request handler that
 * resolves `baseUrl` from the current host and serializes to JSON.
 *
 * Security: an action's `auth` may be a string sentinel (`required` /
 * `optional` / `none`) or an object `{ scheme, scopes? }`. We register only
 * the schemes actually referenced and emit a per-operation `security`
 * requirement that matches — no more "everything is bearer".
 */

import type { Action, Snapshot } from '../types.js';

/** Caller-controlled inputs to {@link snapshotsToOpenApi}. */
...
/**
 * Build an OpenAPI 3.1 document from a snapshot set.
 *
 * Each snapshot becomes a `GET /ahtml/<path>` operation that advertises both
 * compact and JSON media types. Each action with an `execute_url` becomes a
 * separate operation under its declared verb (default `POST`). Returns a
 * plain object — the caller serializes it.
 */
...
/**
 * Map a scheme name to its OpenAPI `securityScheme` shape. Unknown schemes
 * fall back to a generic `http` scheme so sites can declare names OpenAPI
 * doesn't know about (e.g. `"hmac"`) without us erroring out.
 */
function describeScheme(scheme: string): Record<string, unknown> {
  ...
  // Fallback: treat as a custom HTTP scheme.
